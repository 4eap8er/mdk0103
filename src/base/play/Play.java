    ...
    private void battleLoop() {
        while (kingHealth > 0 && units.length > 0 && enemies.length > 0) {
            ...
            while (unit.getHealth() > 0 && enemy.getHealth() > 0) {
                enemy.setHealth(enemy.getHealth() - unit.getDamage());
                ...
                if (enemy.getHealth() <= 0) {
                    ...
                    gold += enemy.getNetWorth();
                    removeEnemy(enemy);
                    break;
                }

                unit.setHealth(unit.getHealth() - enemy.getDamage());
...
class Enemy extends Unit {
    ...
    public Enemy(String name, int health, int damage, int netWorth) {
